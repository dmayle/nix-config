...
rec {
    ...
    concatLists (
      attrValues (
        mapAttrs (
          ...
          else if type == "directory" && pathExists "${toString dir}/${name}/default.nix" then
            [
              {
                inherit name;
                value = dir + "/${name}";
  ...
  loadModules = (
    paths:
    listToAttrs (map (path: nameValuePair (baseNameOf path) (listToAttrs (findModules path))) paths)
  );
}
